    ...
    <head>
        ...
        <?php
            function enregistrer($MailUtilisateur,$NomUtilisateur, $PrenomUtilisateur, $AdresseUtilisateur, $CodePostalUtilisateur, $VilleUtilisateur, $PaysUtilisateur, $DateNaissanceUtilisateur, $TelephoneUtilisateur, $mdp){
                ...
                if($bdd->query("SELECT * FROM Utilisateur WHERE MailUtilisateur='$MailUtilisateur'")->fetch()){
                    $err='mail';echo "<meta http-equiv='refresh' content='2;URL=new.php?mail=$MailUtilisateur&n=$NomUtilisateur&p=$PrenomUtilisateur&adr=$AdresseUtilisateur&cp=$CodePostalUtilisateur&ville=$VilleUtilisateur&anniversaire=$DateNaissanceUtilisateur&pays=$PaysUtilisateur&num=$TelephoneUtilisateur&erreur=$err'>";} 
                else{
        ...
        if($_GET['mail']!="" && $_GET['n']!="" && $_GET['p']!="" && $_GET['adr']!="" && $_GET['cp']!="" && $_GET['ville']!="" && $_GET['anniversaire']!=""&& $_GET['pays']!="" && $_GET['num']!=""&& $_GET['mdp1']!="" && $_GET['mdp2']!="" && $_GET['mdp2']==$_GET['mdp1']) {   
            ...
            // erreur vaut $err='mdp'
            $err='mdp';
            echo "<meta http-equiv='refresh' content='2;URL=new.php?mail=$MailUtilisateur&n=$NomUtilisateur&p=$PrenomUtilisateur&adr=$AdresseUtilisateur&cp=$CodePostalUtilisateur&ville=$VilleUtilisateur&anniversaire=$DateNaissanceUtilisateur&pays=$PaysUtilisateur&num=$TelephoneUtilisateur&erreur=$err'>";}
        ?>
